      ...
      <>
        ...
        <div className="pagination-controls">
          ...
          <div className="pagination-left">
            {!hideBackButton && (
              <button
                // Locate your back-button pagination callback block:
                onClick={() => {
                  if (currentPage === 1) {
                    const tabs = Object.keys(tabPages);
                    const prevIndex =
                      ...
                      tabs.length;
                    const standardPrev = tabs[prevIndex];

                    // Explicitly validate computed layout tabs against the allowlist right inside navigation
                    const safePrev = ALLOWED_TABS.has(standardPrev)
                      ? standardPrev
                      : "Templates";
      ...
      <div className="view-assessment-container container p-0 overflow-hidden">
        <div className="d-flex flex-column flex-lg-row w-100">
          <div className="tabs view-assessment-tabs col-12 col-lg-3">
            ...
            {
              // Locate inside your sidebar/tab layout button render mapping expression:
              Object.keys(tabPages).map((tab, idx) => {
                ...
                return (
                  <button
                    key={`tab-${tab}-${idx}`}
                    className={`tab ${isActive ? "active" : ""} ${isVisited ? "focus" : ""} ${!isUnlocked ? "tab-disabled" : ""}`.replace(
                      /\s+/g,
                      " ",
                    )}
                    onClick={() => {
                      ...
                      // Assert allowlist checking directly at the event execution sink
                      const safeTabClick = ALLOWED_TABS.has(tab)
                        ? tab
                        : "Templates";
                      navigate(`?tab=${safeTabClick}`);
                    }}
                    disabled={!isUnlocked}
                    title={
                      isUnlocked
