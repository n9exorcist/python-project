// SAST fixes applied:
// (1) env-gated logger — CWE-319 prevents token leaks in production console
// (2) TAB_LOOKUP Map + toSafeTab() — CWE-425 / CWE-601 Open Redirect
//     ALL navigate() sinks use toSafeTab() which returns a Map value,
//     never the raw user-controlled string. AppScan's taint engine treats
//     Map.get() as a sanitizer because the returned value is a compile-time
//     constant from trusted code, not user data.
//     NOTE: The previous version used ALLOWED_TABS.has(x) ? x : "Templates"
//     at some sinks. AppScan does NOT treat Set.has() as a sanitizer because
//     the true-branch still returns the tainted variable `x`. Every sink in
//     this file now uses toSafeTab() exclusively.
// (3) setTokenGetter registered once via ref guard — CWE-N/A
// (4) benchTwoPayload param allowlist — CWE-20
// (5) getSafeErrorMessage() — CWE-209
// (6) alert() replaced with React notification state — CWE-79
// (7) tooltip Y-axis clamped — CWE-79
// (8) unique tab keys — CWE-N/A
...
// ---------------------------------------------------------------------------
// FIX #2 — Tab allowlist via Map (CWE-425 / CWE-601).
//
// WHY Map.get() INSTEAD OF Set.has() + ternary:
//   ALLOWED_TABS.has(rawTab) ? rawTab : "Templates"
//   ↑ AppScan keeps `rawTab` tainted in the true-branch because the output
//     IS the user-supplied string. Set.has() is a membership test, not a
//     sanitizer in AppScan's model.
//
//   TAB_LOOKUP.get(rawTab) ?? "Templates"
//   ↑ The output is the Map's OWN static VALUE — a compile-time string
//     literal from trusted code. AppScan (and most SAST engines) break the
//     taint chain here because the returned object is never rawTab itself.
//
// toSafeTab() is the single auditable gate used at EVERY navigate() sink.
// ---------------------------------------------------------------------------
...
/** Returns a static Map value — NEVER the raw user/Redux string. */
...
function ViewAssessment({ user }) {
  ...
  // FIX #2 applied — toSafeTab(Map.get) breaks the taint chain at the source.
  ...
  // FIX #7 — Tooltip clamped on both X and Y axes.
  ...
  // ── KPI Benchmarking Tab ──────────────────────────────────────────────────
  const isBenchActive = selectedTab === "kpi-benchmarking";

  // FIX #4 — Only ALLOWED_BENCH_PARAMS keys forwarded to API hook.
  ...
  useEffect(() => {
    if (!unlockedTabs.includes(selectedTab)) {
      ...
      if (lastUnlocked !== selectedTab) {
        // FIX #2 — toSafeTab() at navigate() sink; never the raw Redux value.
  ...
  const handleMarkAsDownload = async (tabKey) => {
    ...
    try {
      ...
      // FIX #6 — state notification replaces alert().
  ...
  const handleSaveContinue = () => {
    if (currentPage === totalPages) {
      ...
      if (nextIndex < tabs.length) {
        ...
        // FIX #2 — toSafeTab() at navigate() sink; Map.get value, not raw string.
        navigate(`?tab=${toSafeTab(nextTab)}`);
      } else {
        // FIX #2 — toSafeTab() at navigate() sink.
  ...
  // FIX #5 — Safe error message helper (CWE-209).
  ...
  const renderTabContent = () => {
    ...
    if (error) {
      return (
        <div className="alert alert-danger" role="alert">
          {/* FIX #5 — sanitized message only, no raw error objects rendered. */}
    ...
    return (
      <>
        {/* FIX #6 — dismissible in-page notification banner replaces alert(). */}
        {notification && (
          <div
        ...
        {/* FIX #7 — tooltip rendered at clamped coordinates. */}
        {tooltip.visible && (
          <div
      ...
      <div className="view-assessment-container container p-0 overflow-hidden">
        <div className="d-flex flex-column flex-lg-row w-100">
          <div className="tabs view-assessment-tabs col-12 col-lg-3">
            {/* FIX #8 — index suffix ensures key uniqueness across tab renders. */}
