  ...
  return (
    <div className="text-xs font-mono">
      ...
      <div className="flex gap-4 flex-wrap mb-3 p-2 bg-gray-800 rounded">
        ...
        <span>
          Task count: <strong className="text-green-400">{tasks.length}</strong>
        </span>
        <span>
          Last Δ:{" "}
          <strong
            className={taskDelta >= 0 ? "text-green-400" : "text-red-400"}
          >
      ...
      <table className="w-full border-collapse">
        ...
        <tbody>
          {renderLog.map((row, i) => (
            <tr
              key={i}
              className={`border-b border-gray-800 ${i === 0 ? "text-black" : "text-gray-400"}`}
            >
              <td className="py-0.5 pr-4">{row.n}</td>
              <td className="py-0.5 pr-4">{row.time}</td>
              <td className="py-0.5 pr-4">{row.tasks}</td>
              <td
                className={`py-0.5 pr-4 ${row.delta > 0 ? "text-green-400" : row.delta < 0 ? "text-red-400" : "text-gray-500"}`}
              >
          ...
          {renderLog.length === 0 && (
            <tr>
              <td colSpan={5} className="text-gray-600 py-2">
                No renders recorded yet — add or remove a task.
              </td>
            </tr>
          )}
        </tbody>
